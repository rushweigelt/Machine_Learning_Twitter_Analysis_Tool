...
HUMAN_DATA = r'G:\Documents\Drexel\Final_Project\Datasets\cresci_2017_dataset\cresci-2017.csv\datasets_full.csv\genuine_accounts.csv\genuine_accounts.csv'

COLS = ['retweet_count', 'like_count', 'reply_count', 'hashtag_count', 'url_count', 'mention_count', 'followers_count', 'following_count']


def get_train_test_split(dataset, labels):
    """
    Regular train test split function
    """
    dataset['bot'] = labels

    user_id_list = np.unique(dataset['userid'].values)

    train_users, test_users = train_test_split(user_id_list, shuffle=True)

    train_set = dataset.loc[dataset['userid'].isin(train_users)]
    test_set = dataset.loc[dataset['userid'].isin(test_users)]
